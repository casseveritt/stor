...
- Registry endpoints that act on a specific node: `/suspend/{node_id}`, `/nodes/{node_id}` — not `/{handle}`
- Notification records: store `post_node_id`, `author_node_id` — not server URLs
- Signature messages: `f"contacc:suspend:{node_id}:{timestamp}"` — not handle
