...
def _row_to_comment(row) -> dict:
    id_, content_hash, asset_id, parent_id, author_node_id, body, created_at, pred, succ, deleted, author_node_id_2 = row
    return {
        "id": id_,
        "content_hash": content_hash,
        "asset_id": asset_id,
        "parent_id": parent_id,
        "author_node_id": author_node_id_2,
...
def fetch_comments(asset_id: str, request: Request, identity: OptionalAuthDep):
    ...
    rows = db.execute(
        """SELECT c.id, c.content_hash, c.asset_id, c.parent_id, c.author_node_id,
...
async def post_comment(asset_id: str, payload: _PostCommentBody, request: Request, identity: FederatedOrTokenDep):
    db = request.app.state.db
    _require_asset_acl(db, asset_id, identity)
    ...
    now = now_ns()
    author_node_id = identity.node_id  # None when owner

    db.execute(
        """INSERT INTO comments
             (id, content_hash, asset_id, parent_id, author_node_id,
              body, created_at, predecessor, successor, deleted)
           VALUES (?, ?, ?, ?, ?, ?, ?, NULL, NULL, 0)""",
        (comment_id, content_hash, asset_id, payload.parent_id, author_node_id, payload.body, now),
    )
    db.commit()
...
def request_comment_edit(
    ...
    row = db.execute(
        "SELECT author_node_id FROM comments WHERE id = ? AND asset_id = ? AND deleted = 0",
        (comment_id, asset_id),
    ).fetchone()
    if row is None:
        raise HTTPException(status_code=404, detail="Comment not found")

    author_node_id = row[0]
    caller_is_author = (identity.is_owner and author_node_id is None) or (
        not identity.is_owner and identity.node_id == author_node_id
    )
    ...
    db.execute(
        """INSERT INTO comment_edit_requests
             (id, comment_id, requester_node_id, new_body, created_at, status)
           VALUES (?, ?, ?, ?, ?, 'pending')""",
        (request_id, comment_id, identity.node_id, payload.new_body, now_ns()),
    )
    db.commit()
...
def approve_edit(db, request_id: str) -> dict:
    ...
    old = db.execute(
        "SELECT asset_id, parent_id, author_node_id FROM comments WHERE id = ?",
        (comment_id,),
    ).fetchone()
    if old is None:
        raise ValueError("Comment not found")
    asset_id, parent_id, author_node_id = old
        ...
        db.execute(
            """INSERT INTO comments
                 (id, content_hash, asset_id, parent_id, author_node_id,
                  body, created_at, predecessor, successor, deleted)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL, 0)""",
            (new_id, _body_hash(new_body), asset_id, parent_id, author_node_id,
