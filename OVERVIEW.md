...
## Relationship to contacc

The federated node network built on top of `stor` — including the federation protocol, node identity, ACLs, feeds, credentials, and watermarking — is a separate layer referred to as **contacc**. `stor` is contacc's storage backend. See `doc/spec.md` and related documents for the contacc protocol.
